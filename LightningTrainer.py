...
from src.network import encoder, generator, discriminator
from src.loss.perceptual_similarity import perceptual_loss as ps
from torch.utils.tensorboard import SummaryWriter
from default_config import ModelModes, ModelTypes

class HiFiC(pl.LightningModule):
    def __init__(
            self,
            args=None,
        ...
        self.Generator = generator.Generator(self.image_dims, self.batch_size, C=self.args.latent_channels,
                                             ...
                                             channel_norm=self.args.use_channel_norm, sample_noise=
        ...
        ### lightnig part of training step
        self.current_D_steps, self.train_generator = 0, True
        self.best_loss, self.best_test_loss, self.mean_epoch_loss = np.inf, np.inf, np.inf
        self.start_time = time.time()

        self.train_writer = SummaryWriter(os.path.join(args.tensorboard_runs, 'train'))
        self.test_writer = SummaryWriter(os.path.join(args.tensorboard_runs, 'test'))

        self.Disc_out = namedtuple("disc_out",
                                   ["D_real", "D_gen", "D_real_logits", "D_gen_logits"])

    def optimize_loss(self, loss, opt, retain_graph=False):
        self.manual_backward(loss, opt, retain_graph=False)
        opt.step()
        opt.zero_grad()

    def optimize_compression_loss(self, compression_loss, gen_opt):
        self.manual_backward(compression_loss, gen_opt)
        gen_opt.step()
        gen_opt.zero_grad()
    ...
    def compression_forward(self, x):
        ...
        dct = {'input_image': x, 'reconstruction': reconstruction, 'latents_quantized': latents_quantized,
               'n_bpp': total_nbpp, 'q_bpp': total_qbpp}

        return dct, hyperinfo
    ...
    def discriminator_forward(self, intermediates, train_generator):
        ...
        return self.Disc_out(D_real, D_gen, D_real_logits, D_gen_logits)
    ...
    def compression_loss_f(self, intermediates, hyperinfo):
        ...
        distortion_loss = self.distortion_loss(x_gen.float(), x_real.float())
        ...
        # weighted_distortion = weighted_distortion.type_as(weighted_rate)
        weighted_R_D_loss = weighted_rate + weighted_distortion
        # weighted_perceptual = weighted_perceptual.type_as(weighted_R_D_loss)
        weighted_compression_loss = weighted_R_D_loss + weighted_perceptual

        intermediates["distortion"] = distortion_loss
        intermediates["perceptual_loss"] = perceptual_loss
    ...
    def forward(self, x, train_generator=False, return_intermediates=False, writeout=True, greedy=True):
        ...
        compression_model_loss = self.compression_loss_f(intermediates, hyperinfo)
    ...
    def validation_step(self, data_batch, batch_nb, dataloader_idx):
        data, bpp = data_batch
        print("validation", self.args.figures_save)
        if dataloader_idx == 0:
            losses, intermediates = self(data, return_intermediates=True, writeout=False)
            utils.save_images(self.train_writer, self.step_counter, intermediates["input_image"],
                              intermediates["reconstruction"],
                              fname=os.path.join(self.args.figures_save,
                                                 'recon_epoch{}_idx{}_TRAIN_{:%Y_%m_%d_%H:%M}.jpg'.format(
                                                     self.current_epoch, batch_nb,
                                                     datetime.datetime.now())))

            distortion_loss = intermediates["distortion"]
            perceptual_loss = intermediates["perceptual_loss"]
            n_bpp = intermediates["n_bpp"]
            q_bpp = intermediates["q_bpp"]

            self.log('TRAIN-distortion', distortion_loss.item())
            self.log('TRAIN-perceptual', perceptual_loss.item())
            self.log('TRAIN-n_bpp', n_bpp)
            self.log('TRAIN-q_bpp', q_bpp)

        elif dataloader_idx == 1:
            losses, intermediates = self(data, return_intermediates=True, writeout=True)
            utils.save_images(self.test_writer, self.step_counter, intermediates["input_image"],
                              intermediates["reconstruction"],
                              fname=os.path.join(self.args.figures_save,
                                                 'recon_epoch{}_idx{}_TEST_{:%Y_%m_%d_%H:%M}.jpg'.format(
                                                     self.current_epoch, batch_nb,
                                                     datetime.datetime.now())))
            distortion_loss = intermediates["distortion"]
            perceptual_loss = intermediates["perceptual_loss"]
            ...
            self.log('TEST-distortion', distortion_loss.item())
            self.log('TEST-perceptual', perceptual_loss.item())
            self.log('TEST-n_bpp', n_bpp)
            self.log('TEST-q_bpp', q_bpp)
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.use_discriminator is True:
            ...
            losses = self.forward(data, train_generator=self.train_generator)
            self.compression_loss = losses['compression']
            disc_loss = losses['disc']

            if self.train_generator is True:
                self.optimize_compression_loss(self.compression_loss, gen_opt)
                self.train_generator = False
            else:
                self.optimize_loss(disc_loss, disc_opt)
                self.current_D_steps += 1

                if self.current_D_steps == self.args.discriminator_steps:
            ...
            losses = self.forward(data, train_generator=True)
            self.compression_loss = losses['compression']
            self.optimize_compression_loss(self.compression_loss, gen_opt)

        self.log('compression', losses['compression'].item(), on_step=True, on_epoch=True, prog_bar=True, logger=True)
        return

    def configure_optimizers(self):
        amortization_parameters = itertools.chain.from_iterable(
            [am.parameters() for am in self.amortization_models])

        hyperlatent_likelihood_parameters = self.Hyperprior.hyperlatent_likelihood.parameters()
        hyperlatent_likelihood_with_amortization_opt = torch.optim.Adam([
            {'params': amortization_parameters},
            {'params': hyperlatent_likelihood_parameters}
        ], lr=self.args.learning_rate)

        optimizers = [hyperlatent_likelihood_with_amortization_opt]
        lambda1 = lambda epoch: 0.95 ** epoch

        scheduler = [LambdaLR(hyperlatent_likelihood_with_amortization_opt, lr_lambda=lambda1)]

        if self.use_discriminator is True:
            discriminator_parameters = self.Discriminator.parameters()
            disc_opt = torch.optim.Adam(discriminator_parameters, lr=self.args.learning_rate)
            optimizers.append(disc_opt)
            scheduler.append(LambdaLR(disc_opt, lr_lambda=lambda1))
